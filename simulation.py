...
####calculate traffic density (number of cars) in a given direction
def traffic_light_weight(signal):
    weight = 0
    for vehicle in simulation:
        if vehicle.direction_number == signal and vehicle.crossed == 0:
            if signal == 0:
                weight += 1
            if signal == 1:
                weight += 1
...
def repeat():
    ...
    for i in range (0,4):
        weight[i] = traffic_light_weight(i)
        if weight[i] > weight[max_signal]:
            max_signal = i
    print(weight)
    nextGreen = max_signal
    ####

    if nextGreen != currentGreen:
        currentYellow = 1   # set yellow signal on
        signals[currentGreen].yellow = 2    #TEMPO LUZ AMARELA
    ...
    signals[currentGreen].green = 4 # TEMPO MINIMO LUZ VERDE   #int(weight[max_signal]*traffic_cycle) #defaultGreen[currentGreen]
...
def generateVehicles():
    while(True):
        ...
        time.sleep(1)  #INTERVALO ENTRE CARROS
